...
class Expression {
    ...
    fun getCleanExpression(calculation: String): String {
        ...
        if (cleanCalculation.contains('∛')) {
            cleanCalculation = formatCubeRoot(cleanCalculation)
        }
        if (cleanCalculation.contains('!')) {
        ...
        Log.e("expression", cleanCalculation)
    ...
    private fun replaceSymbolsFromCalculation(calculation: String): String {
        ...
        if (calculation2.contains('|')) {
            calculation2 = formatAbsolute(calculation2)
        }
        calculation2 = calculation2.replace('÷', '/')
        calculation2 = calculation2.replace("φ", "(1+√5)/2")
        ...
        calculation2 = calculation2.replace("sinh⁻¹", "arcsh")
        calculation2 = calculation2.replace("cosh⁻¹", "arcch")
        calculation2 = calculation2.replace("tanh⁻¹", "arcth")
    ...
    private fun formatCubeRoot(calculation: String): String {
        // Replace ∛5 by cbrt(5)
        var cleanCalculation = calculation
        var parenthesisOpened = 0
    ...
    private fun formatAbsolute(calculation: String): String {
        var cleanCalculation = calculation
        var isAbsOpening = false

        while (true) {
            val index = cleanCalculation.indexOf("|")
            if (index != -1) {
                cleanCalculation = if (!isAbsOpening) cleanCalculation.replaceFirst("|", "abs(")
                else cleanCalculation.replaceFirst("|", ")")
                isAbsOpening = !isAbsOpening
            } else break;
        }
